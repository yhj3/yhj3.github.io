...
\begin{itemize}
\item Built a pipeline that probes text classifiers by having LLaMA-2 rewrite whole sentences rather
      than swapping words as TextFooler and PWWS do, then asked what such a pipeline is measuring:
      across 32 configurations, rewrites of already-successful attacks still fool the classifier only
      44.5\% of the time, buying fluency (median perplexity 22.9 vs.\ 522) at the price of more than
      half the attacks.
\item Identified and quantified three ways a generated probe is counted without being a valid test.
      \textbf{Label validity}: a prompt ablation on identical data (119/180 vs.\ 58/180,
      $p = 1.7\times10^{-10}$) showed the best-performing prompt wins by instructing the model to
      change the label, so reporting flip rate alone overstates attack success by $3.05\times$
      overall and $19.5\times$ on topic classification.
\item \textbf{Input validity}: on sentence-pair tasks most probes collapse the two segments and are
      not well-formed inputs; because malformed probes are scored as failures, leaving them in
      understates attack success (12.5\% vs.\ 41.7\%), so the two errors push in opposite directions
      and do not cancel.
\item \textbf{Measurement validity}: audited my own earlier version of this study, in which the
      classifier had never been run on the generated text, and derived the one-line check that
      catches it; the paper closes with three numbers any red-teaming report should carry.
\end{itemize}
